...
const AdList = () => {
  ...
  const [deleteTarget, setDeleteTarget] = useState<AdDto | null>(null);
  const [error, setError]               = useState<string | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ['ads', page, pageSize, search],
    queryFn:  () => getAds(page + 1, pageSize, search || undefined),
  });
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'altTag', headerName: 'Alt Tag' },
    { field: 'category', headerName: 'Category' },
    { field: 'clicks', headerName: 'Clicks' },
    { field: 'pageViews', headerName: 'Page Views' },
    {
      field: 'startDate',
      ...
      getActions: (params) => {
        const actions = [
          <GridActionsCellItem
            icon={<EditIcon />}
            label="Edit"
            onClick={() => navigate(`/ads/${params.row.id}/edit`)}
          />,
        ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        <h1 className="text-xl font-semibold text-neutral-800 mr-auto">Ads</h1>
