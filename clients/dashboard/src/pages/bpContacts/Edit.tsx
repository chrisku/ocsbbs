...
import { getBpContactById, updateBpContact } from '../../api/bpContacts';
import { getBpCompanies } from '../../api/bpCompanies';
import type { UpdateBpContactDto } from '../../types';
...
const EditBpContact = () => {
  usePageTitle('Edit BP Contact');
  const navigate    = useNavigate();
  const { id }      = useParams<{ id: string }>();
  const queryClient = useQueryClient();

  const [form, setForm]   = useState<UpdateBpContactDto | null>(null);
  const [error, setError] = useState<string | null>(null);

  const { data: contact, isLoading } = useQuery({
    queryKey: ['bpContacts', id],
    queryFn:  () => getBpContactById(Number(id)),
  });
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      state:       contact.state ?? '',
      zip:         contact.zip ?? '',
      bpCompanyId: contact.bpCompanyId,
    });
  }, [contact]);

  const mutation = useMutation({
    mutationFn: (dto: UpdateBpContactDto) => updateBpContact(Number(id), dto),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['bpContacts'] });
      navigate('/bp-contacts');
    },
    onError: (err: any) => {
      setError(err.response?.data?.message || 'Failed to update BP contact');
    },
  });
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-semibold text-neutral-800">Edit BP Contact</h1>
      </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-lg p-6">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4" autoComplete="off">
          ...
          <div className="flex flex-col gap-1">
            ...
            <ImageUpload
              value={form.photo ?? ''}
              onChange={url => setForm(prev => prev ? ({ ...prev, photo: url }) : prev)}
